...
class ProductCatalog(searchService: SearchService) extends Actor {
  ...
  override def receive: Receive = {
    case GetItems(brand, productKeyWords) => {
      //println("product catalog actor path: ", self.path)
      val res = Items(searchService.search(brand, productKeyWords))
      sender() ! res
    }
  }
}
...
object ProductCatalogApp extends App {
  ...
  private val productCatalogSystem = ActorSystem("ProductCatalog",
    config.getConfig("productcatalog").withFallback(config))

  val catalogActor = productCatalogSystem.actorOf(ProductCatalog.props(new SearchService()), "productcatalog")
//  catalogActor ! GetItems("gerber", List("cream"))
